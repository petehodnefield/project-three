...
db.once('open', async () => {
    ...
    for (let i = 0; i < 50; i += 1) {
        ...
        const organization = organizations[Math.floor(Math.random() * 5 )]
        userData.push({username, email, password, organization})
    }
    ...
    for (let i = 0; i < 50; i += 1) {
        ...
        const randomEventIndex = Math.floor(Math.random() * createdEvents.ops.length);
